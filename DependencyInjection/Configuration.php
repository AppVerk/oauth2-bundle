    ...
    private function createAuthorizationServerNode(): NodeDefinition
    {
        ...
        // @TODO Remove in v4 start
                ...
                ->scalarNode('auth_code_ttl')
                    ...
                    ->cannotBeEmpty()
                    ->setDeprecated('trikoder/oauth2-bundle', '4.0.0', '"%path%.%node%" is deprecated, use "%path%.grant_types.authorization_code.auth_code_ttl" instead.')
                    ->beforeNormalization()
                        ->ifNull()
                        ->thenUnset()
                    ->end()
                ...
                ->booleanNode('require_code_challenge_for_public_clients')
                    ...
                    ->setDeprecated('trikoder/oauth2-bundle', '4.0.0', '"%path%.%node%" is deprecated, use "%path%.grant_types.authorization_code.require_code_challenge_for_public_clients" instead.')
                    ->beforeNormalization()
                        ->ifNull()
                        ->thenUnset()
                    ->end()
            ...
            $node
                ->children()
                    ->booleanNode(sprintf('enable_%s_grant', $oldGrantType))
                        ...
                        ->setDeprecated('trikoder/oauth2-bundle', '4.0.0', sprintf('"%%path%%.%%node%%" is deprecated, use "%%path%%.grant_types.%s.enable" instead.', $grantType))
                        ->beforeNormalization()
                            ->ifNull()
                            ->thenUnset()
                        ->end()
